...
import { SceneRenderer } from 'svg3d-lib';
import { AxesGizmo, GridHelper, ViewportGizmo } from '../Gizmos';
import { useAppContext } from '../../context/AppContext';
import ViewportTopBar from './ViewportTopbar';
...
/**
 * 3D Viewport component that renders a scene using the scene graph and camera system
 */
function Viewport3D({ viewportId, isActive = false }) {
  // Get context
  const {
    sceneManager,
    isLoading,
  ...
  // Get this viewport's data
  const viewport = viewports.find(vp => vp.id === viewportId);
  if (!viewport) {
    console.error(`Viewport with id ${viewportId} not found`);
    return <div className="viewport-error">Viewport not found</div>;
  }

  const { cameraManager, settings, name } = viewport;
  ...
  useEffect(() => {
    if (rendererRef.current && settings) {
      console.log(`Updating renderer settings for viewport ${viewportId}:`, settings);
      rendererRef.current.setSettings(settings);
      
      // Update light direction if it's in the settings
      if (settings.lightDirection) {
        rendererRef.current.setLightDirection(settings.lightDirection);
      }
    }
  }, [settings, viewportId]);
  ...
  const handleClick = (event) => {
    ...
    // Make this viewport active
    handleViewportClick();
    
    // Model selection via hit-testing would be implemented here
    console.log(`Viewport ${viewportId} clicked at`, event.clientX, event.clientY);
  };
  ...
  const handleUpdateSettings = (newSettings) => {
    updateViewportSettings(viewportId, newSettings);
  };

  // Toggle functions for gizmos
  const toggleGrid = () => setShowGrid(!showGrid);
  const toggleAxes = () => setShowAxesGizmo(!showAxesGizmo);
  ...
  return (
    ...
    >
      <ViewportTopBar 
        ...
        viewportName={name}
        showGrid={showGrid}
        showAxes={showAxesGizmo}
        toggleGrid={toggleGrid}
        toggleAxes={toggleAxes}
        toggleHelp={toggleHelp}
        viewportIndex={parseInt(viewportId.split('-')[1]) - 1}
      />
      ...
      <div className="viewport-container">
        ...
        {isActive && <FPSCounter fps={fps} />}
        {isActive && <StatsDisplay stats={rendererStats} />}
        {isActive && activeCamera && <CameraInfo camera={activeCamera} />}
        ...
        {isActive && selectedModelId && <div className="selection-indicator">Selected: {selectedModelId}</div>}
      </div>
    </div>
  );
}
